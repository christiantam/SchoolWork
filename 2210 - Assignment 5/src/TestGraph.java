...
public class TestGraph {
	...
	try {
        ...
        /* Degrees of the nodes in test graph */
	...
	for (i = 0; i < 9; ++i)
	    for (j = 0; j < i; ++j)
		if (M[i][j] == 1) G.insertEdge(V[i],V[j],"wall");
		else if (M[i][j] == 2)G.insertEdge(V[i],V[j],"hall");
	}
	catch (GraphException e) {
	...
	try {
		...
		for (j = 0; j < i; ++j) {
			...
			s = uv.getType();
			if ((M[i][j] == 1 && s.compareTo("wall") != 0) ||
			    (M[i][j] == 2 && s.compareTo("hall") != 0)) failed = true;
		    }
		}
        ...
        try {
            for (i = 0; i < numNodes; ++i) {
                ...
                while (neighbours.hasNext()) {
                    ...
                    else if (("wall".compareTo(uv.getType()) == 0) && (M[node1][node2] != 1)) {
                        System.out.println("    Error: There should not be a wall between");
                        System.out.println("           nodes "+node1+" and "+node2);
                        failed = true;
                    }
                    else if (("hall".compareTo(uv.getType()) == 0) &&(M[node1][node2] != 2)) {
                        System.out.println("    Error:There should not be a hall between");
                        System.out.println("          nodes "+node1+" and "+node2);
                        failed = true;
                    }
                }
